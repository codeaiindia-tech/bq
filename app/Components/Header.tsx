...
const Header = ({ contactButton, menuBar }: { contactButton: boolean, menuBar: boolean }) => {
    return (
        <div
            className='w-full py-8 px-16 flex justify-between items-center'
        >
            <span className=' text-center  text-blue-500 text-3xl font-bold' >
                Medi
                <span className='text-orange-400' >Care+</span>
            </span>
            <div>
                <ul className='hidden sm:flex items-center justify-center gap-x-8'>
                ...
                {menuBar && <span className=' flex sm:hidden' > <Menu /> </span>}
            </div>
        </div>
    )
}
